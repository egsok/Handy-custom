...
async benchmarkTranscriptionFile(filePath: string, warmupPath: string | null, runsPerCondition: number | null, skipModels: string[] | null, maxChunkSecsOverride: number | null, language: string | null, overrides: BenchmarkOverrides | null) : Promise<Result<string, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("benchmark_transcription_file", { filePath, warmupPath, runsPerCondition, skipModels, maxChunkSecsOverride, language, overrides }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
